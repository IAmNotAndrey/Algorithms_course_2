...
def num_of_combs_via_od_matrix_2(n, k):
    if n < k:
        raise Exception('n < k')
    if n < 0 or k < 0:
        raise Exception('n or k < 0')
...
# 		array[i] = array[i - 1] * (i + difference) // i
# 	return array[-1]
...
if __name__ == '__main__':
    ...
    for _ in range(test_num):
        ...
        start = time.perf_counter()
        c = num_of_combs_via_od_matrix_2(n, k)
        end = time.perf_counter()
        ...
        if abs(dr.recur_relation(n, k) - num_of_combs_via_od_matrix_2(n, k)) <= 0.1 \
    ...
    match task:
        ...
        case 2:
            ...
            print(
                f'Fastest: {min(mean(iter_od_times), mean(iter_td_times), mean(rec_times))}')
