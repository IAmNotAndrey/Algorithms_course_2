...
def best_investment_enum(investments_: list, project_matrix_: list):
	investments: list = sorted(copy(investments_))
	project_matrix: list = sorted(copy(project_matrix_))

	# investments.insert(0, 0)

	m = len(project_matrix[0])

	def fi(i, x):
		'''Функция выигрыша'''
		# Возникает ошибка, когда investments.index(0), т.к. 0 нет в investments
		try:
			W_result = project_matrix[investments.index(x)][i]
		except ValueError: 
			W_result = 0
		except IndexError: 
			W_result = 0
	...
	variants : dict = {}
	for variant in product(investments+[0], repeat=m):
		if sum(variant) == investments[-1]:
			W = 0
			for i, x in enumerate(variant):
				W += fi(i, x)
			variants[W] = variant
	
	maximum = max(variants.keys())
	return maximum, variants[maximum]
...
if __name__ == '__main__':
	...
	print()
	print(best_investment_enum(investments, project_matrix))
